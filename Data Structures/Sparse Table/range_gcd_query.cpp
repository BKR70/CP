    ...
    for(int i=0; i<=logs[n]; i++) {
        ...
        for(int j=0; j+curLen<=n; j++) {
            if(curLen == 1) {
                ...
                /// precalculate for gcd queries
                table[i][j] = __gcd(table[i-1][j],table[i-1][j+(curLen/2)]);
            }
        }
    }

}
int getValue(int l,int r) {                                   /// query for gcd value in this range.
    int p = logs[r-l+1];
    int pLen = 1 << p;
    return __gcd(table[p][l],table[p][r-pLen+1]);
}
